...
- `維護工具` 子選單：
  ...
  - `資料遷移（不批次同步）`：只補齊/遷移系統欄位，例如將舊 `日曆eventID` 搬到 `CalendarEventId`，並重新隱藏與保護系統欄。
  ...
  - `清除舊事件`：固定針對 `OP` 工作表的 `CalendarEventId` 欄，刪除對應日曆事件；成功後清空該 ID 與同列 `CalendarSheetWriteUpdated`。
  - `版本：2026.05.09`：顯示目前部署版本。

## 功能作用範圍

- 固定針對 `OP` 工作表：`輸出日期`、`輸出一週`、`歸人整合`、`日期排序`、`初始化表格`、`完整安裝／批次同步`、反向同步、`清除舊事件`。
- 針對目前工作表但限定 `OP`：`複製列`。
- 全域設定或 trigger：`設定日曆`、`資料遷移`、`安裝同步`、`安裝反向同步`、`安裝自動輸出`、版本顯示。
...
- 更換 Calendar ID 只會重建雙向同步綁定，不會自動把既有列批次同步到新日曆；若需要同步既有資料，請再執行「完整安裝／批次同步」。
...
- 日曆事件標題格式為 `病歷號 | 姓名 | Condition`；中間欄位會保留位置，姓名空白時會寫成 `病歷號 |  | Condition`，避免反向同步把 Condition 誤判成姓名。分隔符 `|` 前後空白不影響解析，例如 `123|王小明|Cataract` 也可辨識，輸入全形 `｜` 也會自動視為分隔符。
...
- Sheet 寫入 Calendar 後，系統會把 Calendar 回傳的 `updated` 記錄到 `CalendarSheetWriteUpdated`；Calendar 反向同步會略過較舊的 Sheet-originated 回音，同 timestamp 則需事件內容也等同目前 Sheet 投影才會略過，因此「尚未決定時間」的全天事件仍會顯示在日曆上，但不會把使用者稍後補上的時間清空。
...
- 清除舊日曆事件工具固定從 `OP` 工作表的 `CalendarEventId` 欄第 2 列開始處理；若尚未遷移，仍會相容舊 `日曆eventID` 欄。刪除成功或事件已不存在時，會清空該 ID 與同列 `CalendarSheetWriteUpdated`，不會更動病歷號、姓名、日期、時間、Plan、心得等同列資料。
...
- `清除舊事件` 只處理目前試算表內固定 `OP` 工作表的 `CalendarEventId` 欄，並刪除已設定 Calendar ID 內對應事件；使用前請確認該日曆是專用手術日曆。
